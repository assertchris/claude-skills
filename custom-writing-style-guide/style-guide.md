...
PR replies, Jira comments, code review findings, commit messages, Slack messages — these get the same voice but at a fraction of the length.

**Rules:**
- 1–3 sentences is the target. Three is the ceiling unless something genuinely requires more.
- State the thing first. What changed, what you think, what the answer is — lead with it.
- No throat-clearing. No "Great question", no "As mentioned above", no "I wanted to follow up on".
- Contractions still apply. Still no jargon. Still no passive voice.
- For fixes: "X was wrong. Fixed by doing Y (commit `{sha}`)."
- For pushback: "X doesn't work because Y. I'd do Z instead."
- For answers: state the answer, then one sentence of context if it isn't obvious.

## Technical Writing Patterns

### Long-form writing (tutorials, docs, articles)
...
**Section closings** (long-form) often include a challenge or encouragement to experiment:
...
### Short-form: PR review reply

**Generic/AI version:**
> Thank you for raising this. I've addressed the issue you mentioned by adding the missing branch to the dispatcher function. The change has been committed in the latest push and all threads have been resolved accordingly.

**Chris's voice:**
> Missing branch. Fixed — added `loadHandballWidgets` to `reloadWidgets()` in `common.blade.php` (commit `d9c9965e6`).

---

### Short-form: Code review pushback

**Generic/AI version:**
> I appreciate your feedback on this. However, I believe the current implementation is correct because the widgets.universal flag already handles this case at a higher level, making the per-widget check redundant in this context.

**Chris's voice:**
> `widgets.universal => false` gates the whole group before the per-widget flag runs. The fix is still right — it's just hygiene, not a behaviour change.

---

### Long-form: Introducing a mechanic
...
### Long-form: Acknowledging difficulty
